...
nav_order: 7
permalink: /drivers/arrowfebe/licensing/
---

# ArrowFEBE — Licensing

ArrowFEBE is a proprietary Arpeio driver. The binary is free to download, and it
...
> The gate is **always enforced** — there is no build-time off switch, so a
> `.so`/`.dll` lifted out of a host bundle is inert on its own.
...
5. **`arpeio_adbc.lic`** — a file placed next to the driver library
   (`libarrowfebe_adbc_driver.so` / `arrowfebe_adbc_driver.dll`).
...
a Unix epoch. A grace-period warning is also emitted as the licence nears expiry.
...
> Test and CI builds set `-DARROWFEBE_LICENSE_TEST_KEY=ON` and self-mint a token
> offline. A driver built with the test key is never shipped — production
> binaries validate a real Arpeio licence.

## See also

- [`CONNECTION.md`]({{ '/drivers/arrowfebe/connection/#licence' | relative_url }}) — the licence options in the full option reference
- [`TROUBLESHOOTING.md`]({{ '/drivers/arrowfebe/troubleshooting/#licence' | relative_url }}) — the licence error in context
- The [install guide]({{ '/install/' | relative_url }})
