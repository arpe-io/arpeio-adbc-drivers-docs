...
The same recipe in every ADBC client — the driver is loaded **by name** (`arrowfebe`)
...
using AdbcDriver driver = AdbcDriverManager.FindLoadDriver(
    "arrowfebe", loadOptions: AdbcLoadFlags.Default);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut driver = ManagedDriver::load_from_name(
        "arrowfebe", None, AdbcVersion::default(), LOAD_FLAG_DEFAULT, None,
    )?;
...
using AdbcDriver driver = AdbcDriverManager.FindLoadDriver("arrowfebe", loadOptions: AdbcLoadFlags.Default);
using AdbcDatabase database = driver.Open(parameters);
```
...
Bind a parameter to a `$1` placeholder. In the compiled clients you bind an Arrow batch
...
PostgreSQL uses `$1`, `$2`, … placeholders in the extended-query protocol. Array
binding — bind a whole Arrow batch of parameters in one call (Python's `executemany`):
...
in C# via `CAdbcDriverImporter.Load("libarrowfebe_adbc_driver.so", "AdbcDriverInit")`, or by
setting `ARROWFEBE`/`ADBC_DRIVER_PATH` for the name-based loaders. Everything after loading
is identical.
